...
const compare_dates_fn_1 = __importDefault(require("../shared/compare-dates.fn"));
...
class Api {
    ...
    static serializeDate(date) {
        return date_fns_1.lightFormat(date, Api.dateFormat);
    }
    static serializeDateTime(date) {
        return date_fns_1.lightFormat(date, Api.dateTimeFormat);
    }
    static parseDateOrTimeRange(config) {
        ...
        if ('dateRange' in config) {
            ;
            [start, end] = config.dateRange.slice().sort(compare_dates_fn_1.default);
            type = api_types_1.RangeType.Date;
        }
        else {
            ;
            [start, end] = config.timeRange.slice().sort(compare_dates_fn_1.default);
            type = api_types_1.RangeType.DateTime;
        }
        return { type, start, end };
    }
    static serializeRange({ type, start, end, }) {
        return type === api_types_1.RangeType.Date
            ? {
    ...
    call(path, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const params = query_string_1.default.stringify(Object.assign(Object.assign({}, callConfig.params), { api_key: key }));
    ...
    callGenerator(config) {
        return __asyncGenerator(this, arguments, function* callGenerator_1() {
            ...
            while (date_fns_1.isBefore(periodStart, range.end)) {
                const periodEnd = date_fns_1.min([range.end, date_fns_1.add(periodStart, interval)]);
